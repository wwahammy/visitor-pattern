...
{
    /// <summary>
    /// A custom command design-time validation rule for the <see cref="LoadProjects"/> command.
    /// </summary>
    [CommandValidationRule(typeof(LoadProjects))]
    [CLSCompliant(false)]
    public class LoadProjectsValidation : ICommandValidationRule
    {
        private static readonly ITracer tracer = Tracer.Get<LoadProjectsValidation>();
        ...
        public void Validate(ValidationContext context, IAutomationSettingsSchema settingsElement, ICommandSettings settings)
        {
            try
            {
                ...
                //var propValue1 = settings.GetOrCreatePropertyValue(Reflector<LoadProjects>.GetPropertyName(s => s.AProperty), String.Empty);
            ...
            catch (Exception ex)
            {
                ...
                tracer.Error(
                    "Validation rule LoadProjectsValidation unexpectedly failed, error was: '{0}'", ex.Message);
                throw;
            }
        }
    }
